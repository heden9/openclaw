...
export async function add(
  ...
  return await locked(state, async () => {
    ...
    if (existing) {
      ...
      // only the gateway's own convergence touches its declaration key.
      if (systemOwnedDeclarationNamespace && opts?.systemOwned !== true) {
...
async function updateLoadedJob(params: {
  ...
  // Mirrors the add-time boundary: no caller may patch or edit a system-owned
  // declaration; the gateway converges it via add only.
  await ensureLoaded(state, { skipRecompute: true });
  ...
  if (systemOwnedDeclarationKeyNamespace(job.declarationKey)) {
    throw new Error("system-owned monitor jobs cannot be edited by cron clients");
  }
  const now = state.deps.nowMs();
...
export async function remove(
  ...
  const result = await locked(state, async () => {
    ...
    if (
      systemOwnedDeclarationKeyNamespace(removedJob.declarationKey) &&
