...
import { isSystemOwnedCronDeclaration } from "../../cron/system-owned-declaration.js";
...
export async function resolveCronEditPayloadDeliveryPatch(
  ...
  if (hasTimeoutSeconds && !hasCommandSpecificPayloadField && !hasAgentTurnSpecificPayloadField) {
    ...
    if (
      ...
      isSystemOwnedCronDeclaration(existingJob.declarationKey)
    ) {
  ...
  if (
    ...
    const existingJob = await loadExistingJob();
    if (isSystemOwnedCronDeclaration(existingJob.declarationKey)) {
